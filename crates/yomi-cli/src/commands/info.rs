...
pub async fn run(args: &InfoArgs) -> Result<()> {
    ...
    // Эвристику по пропорциям запускаем только по явной просьбе и
    // только когда настоящей разметки не нашлось: она читает размеры
    // всех страниц и может ошибаться.
    let split = match (analysis.split, args.deep) {
        (Some(split), _) => Some(split),
        (None, true) => {
            println!("\nЧитаю размеры страниц...");
    ...
    match split {
        ...
        None if kind == FileKind::Volume && args.deep => {
            println!(
                "\nГраницы глав определить не удалось: ни разметки в файле,\n\
                 ни разворотов, по которым их можно было бы угадать."
            );
        }
        None if kind == FileKind::Volume => {
            ...
            println!(
                "\nРазметки глав внутри нет: файл не содержит ни закладок\n\
                 ...
                 Попробуйте `yomi info --deep`: в части изданий главы\n\
                 начинаются с разворота, и границы видно по пропорциям."
            );
        }
        None => {}
    }
