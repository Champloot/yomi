...
pub struct ReadArgs {
    ...
    /// Открыть конкретный том тайтла, а не то, на чём остановились
    ///
    /// Короткого флага нет: -v занят глобальным --verbose.
    #[arg(long, value_name = "НОМЕР")]
    pub volume: Option<u16>,

    /// Открыть том с нужной главой, сразу на её первой странице
    ///
    /// Короткого флага нет: -c занят глобальным --config.
    #[arg(long, value_name = "НОМЕР")]
...
pub enum LibraryCommand {
    ...
    /// Убрать из библиотеки тайтл или отдельный файл
    Forget {
        /// Название тайтла или путь к файлу
        #[arg(value_name = "НАЗВАНИЕ_ИЛИ_ПУТЬ")]
        target: String,
    },
    /// Очистить библиотеку целиком
    Reset {
        /// Подтверждение. Без него команда только покажет, что будет удалено
        #[arg(long)]
        yes: bool,
    },
